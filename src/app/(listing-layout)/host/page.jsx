...
const HotelDashboard = () => {
  ...
  // Automatic redirect if there are no published properties
  useEffect(() => {
    let redirectTimer;
    // We add a small 1.5s delay to prevent harsh immediate redirects before Redux fully hydrates
    if (
      properties &&
      properties.length >= 0 &&
  ...
  useEffect(() => {
    ...
    if (storedProperty) {
      ...
      const firstPublishedProperty =
        properties.find((p) => p.status === "published") || properties[0];
      setSelectedProperty(firstPublishedProperty);
      localStorage.setItem(
        "selectedProperty",
        JSON.stringify(firstPublishedProperty),
      );
    ...
    return (
      ...
      >
        ...
        <Typography variant="h6" color="text.secondary">
          No published properties found. Redirecting to properties page...
        </Typography>
        <Link
